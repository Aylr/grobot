...
#define loopDelay 100                           // milliseconds between loops (fastest response time)
...
void loop() {
    ...
    // if any of the input pin has changed DO STUFF.
    // NOTE: may require addition of all inputs as well.
    if ( (currentAnalogPin0 != lastAnalogPin0)              // all this mess could be on one long line
            | (currentAnalogPin1 != lastAnalogPin1)
            | (currentToggle != lastToggle)
            | (currentRotateCW != lastRotateCW)
            | (currentRotateCCW != lastRotateCCW)
            | (currentOverride != lastOverride)
            | (currentBallDrop != lastBallDrop) ){
        ...
        if (digitalRead(Override) == LOW){           // Checks if the Override button has been pressed
        ...
        }else if(digitalRead(Rotate_CCW) == LOW){    // Checks if the Rotate_CCW button has been pressed
        ...
        }else if (digitalRead(Rotate_CW) == LOW){    // Checks if the Rotate_CW button has been pressed
        ...
        }else if(digitalRead(Ball_drop) == LOW){     // Checks if the Ball_drop button has been pressed
...
void getCurrentJoystickAndButtons (){
    // get new reading for each input and store in the "current" version of each global variable
    currentAnalogPin0 = analogRead(analogJoystickPinX);
    currentAnalogPin1 = analogRead(analogJoystickPinY);
    currentToggle = digitalRead(toggle);
